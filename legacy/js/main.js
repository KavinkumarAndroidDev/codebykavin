...
import { getFirestore, doc, collection, query, onSnapshot, getDocs, orderBy } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
const APP_DATA_PATH = `artifacts/${appId}/public/data/apps`;
const DEFAULT_FEATURED_SCREENSHOT = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 450'%3E%3Cdefs%3E%3ClinearGradient id='g' x1='0' x2='1' y1='0' y2='1'%3E%3Cstop stop-color='%230f111a'/%3E%3Cstop offset='1' stop-color='%231e1e2f'/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width='800' height='450' rx='32' fill='url(%23g)'/%3E%3Ccircle cx='160' cy='120' r='72' fill='%234ac0ff' fill-opacity='.18'/%3E%3Ccircle cx='640' cy='320' r='88' fill='%23ff6b6b' fill-opacity='.18'/%3E%3Crect x='120' y='110' width='560' height='230' rx='24' fill='none' stroke='%234ac0ff' stroke-opacity='.45' stroke-width='10'/%3E%3C/svg%3E";
const DEFAULT_DETAIL_SCREENSHOT = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 600'%3E%3Cdefs%3E%3ClinearGradient id='g' x1='0' x2='1' y1='0' y2='1'%3E%3Cstop stop-color='%230f111a'/%3E%3Cstop offset='1' stop-color='%231e1e2f'/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width='800' height='600' rx='32' fill='url(%23g)'/%3E%3Crect x='96' y='96' width='608' height='408' rx='28' fill='none' stroke='%234ac0ff' stroke-opacity='.38' stroke-width='10'/%3E%3Ccircle cx='180' cy='170' r='54' fill='%234ac0ff' fill-opacity='.16'/%3E%3Ccircle cx='620' cy='430' r='72' fill='%23ff6b6b' fill-opacity='.16'/%3E%3C/svg%3E";
const DEFAULT_SITE_TITLE = 'CodeByKavin - Explore My Creations';
const DEFAULT_SITE_DESCRIPTION = 'Discover fun, productivity, and experimental apps built by Kavin. A personal app store showcasing various projects.';
const DEFAULT_PROFILE_IMAGE = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 192 192'%3E%3Cdefs%3E%3ClinearGradient id='g' x1='0' x2='1' y1='0' y2='1'%3E%3Cstop stop-color='%234ac0ff'/%3E%3Cstop offset='1' stop-color='%230f111a'/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width='192' height='192' rx='96' fill='url(%23g)'/%3E%3Ccircle cx='96' cy='74' r='34' fill='%23ffffff' fill-opacity='.82'/%3E%3Cpath d='M42 164c8-28 30-42 54-42s46 14 54 42' fill='%23ffffff' fill-opacity='.82'/%3E%3C/svg%3E";
...
const getAppScreenshots = (app) => Array.isArray(app?.screenshots) ? app.screenshots.filter(Boolean) : [];
...
const isFirebaseConfigReady = () => {
    ...
    return requiredValues.every((value) => typeof value === 'string' && value.length > 0 && !value.includes('%%'));
};
...
const updatePageMetadata = (page, app = null) => {
    ...
    } else if (page === 'app-detail' && app) {
        title = `${app.name} | CodeByKavin`;
        description = app.tagline || app.description || DEFAULT_SITE_DESCRIPTION;
    } else if (page === '404') {
        title = 'Page Not Found | CodeByKavin';
        description = 'The page you requested could not be found on CodeByKavin.';
    }
...
function setupFirestoreListeners() {
    ...
    const appsCollectionRef = collection(db, APP_DATA_PATH);
...
async function initFirebase() {
    try {
        ...
        // Keep auth state in sync, but do not block initial reads on sign-in.
        ...
        // Start public data listeners immediately so the UI can render sooner.
        setupFirestoreListeners();
    } catch (error) {
        console.error("Firebase initialization failed:", error);
        contentDiv.innerHTML = `<div class="text-center p-20 text-red-500">
            <h1 class="text-3xl font-bold mb-2">Configuration Error</h1>
            <p>This build is missing the Firebase environment variables required to start the app.</p>
        </div>`;
    }
...
async function ensureAuthenticated() {
    ...
    authInFlightPromise = (async () => {
        ...
        if (!signedInUser) {
            throw new Error('Authentication succeeded without a current user.');
        }
...
const renderHomePage = () => {
    ...
    const featuredScreenshots = getAppScreenshots(featuredApp);
    ...
    return `
        ...
        <section class="mb-16">
            ...
            <div class="card-glass p-8 rounded-3xl grid md:grid-cols-2 gap-8 items-center shadow-soft-dark dark:shadow-neon-primary/30">
                ...
                <div class="order-1 md:order-2 rounded-2xl overflow-hidden shadow-2xl transition duration-300 hover:scale-[1.01] bg-gray-900 aspect-[16/9]">
                    <img
                        src="${featuredScreenshots[0] || DEFAULT_FEATURED_SCREENSHOT}"
                        class="w-full h-full object-cover"
                        alt="${featuredApp.name} app preview"
                        width="800"
                        height="450"
                        loading="eager"
                        fetchpriority="high"
...
const renderAppDetailPage = (app, changelog) => {
    ...
    const updateScreenshot = (isLightbox = false) => {
        const screenshotUrl = screenshots[currentScreenshotIndex] || DEFAULT_DETAIL_SCREENSHOT;
        if (isLightbox) {
            const lightboxImage = document.getElementById('lightbox-image');
        ...
        <div class="grid lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="relative mb-10 card-glass p-2 rounded-2xl shadow-soft-dark overflow-hidden aspect-[4/3] bg-black flex items-center justify-center">
                    ...
                    <img 
                        id="app-screenshot"
                        src="${screenshots[0] || DEFAULT_DETAIL_SCREENSHOT}"
                        alt="${app.name} screenshot"
                        class="max-w-full max-h-full object-contain transition-opacity duration-200 ${screenshots.length > 0 ? 'cursor-zoom-in' : ''}"
                        onclick="${screenshots.length > 0 ? `openLightbox('${screenshots[0]}')` : ''}"
                    >
                    <!-- Navigation Arrows -->
            ...
            <div class="flex flex-col md:flex-row items-start md:space-x-10">
                ...
                <div class="w-full md:w-1/3 mb-8 md:mb-0 flex flex-col items-center">
                    <div class="w-48 h-48 rounded-full bg-accent-primary/20 p-2 mb-4 shadow-xl border-4 border-accent-primary/50 overflow-hidden">
                        <img src="${developer.profileImageUrl || DEFAULT_PROFILE_IMAGE}" class="w-full h-full rounded-full object-cover" alt="${developer.name} Profile Photo">
                    </div>
                    ...
                    <div class="mt-6 space-y-3 w-full">
                        ${getSafeArray(developer.links).map(linkUrl => {
                            const link = getLinkDetails(linkUrl || '');
                            return `
                            <a href="${link.url}" target="_blank" rel="noopener noreferrer" class="flex items-center space-x-3 dark:text-text-primary-dark text-text-primary-light hover:text-accent-primary transition duration-200" aria-label="${link.label}">
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ${getSafeArray(developer.skills).map(skill => `
...
const renderNotFoundPage = () => `
    <div class="text-center p-20">
        ...
        <p class="dark:text-text-secondary-dark mb-6">The page you're looking for might have been removed, had its name changed, or is temporarily unavailable.</p>
        <div class="flex justify-center gap-4">
            ...
            <button onclick="location.hash = '#apps'" class="px-6 py-3 rounded-xl font-bold transition duration-200 border border-accent-primary text-accent-primary hover:bg-accent-primary/10">Go to Apps</button>
        </div>
    </div>
`;
...
const setActiveLink = (page) => {
    ...
    const link = Array.from(document.querySelectorAll('.nav-link')).find(l => l.hash === `#${activePage}`);
...
const showAppDetail = async (appId, isReRender = false) => { // isReRender is not used, but kept for signature consistency
    ...
    if (!isReRender) {
        // Show a brief loading indicator while app details are loading
        contentDiv.classList.add('opacity-0');
    ...
    try {
        ...
        const changelogCollectionRef = collection(db, `${APP_DATA_PATH}/${appId}/changelog`);
    ...
    setTimeout(() => {
        ...
        updatePageMetadata('app-detail', app);
        contentDiv.classList.remove('opacity-0');
    }, 300);
...
window.submitRating = async (appId, rating) => {
    ...
    const appRef = doc(db, `${APP_DATA_PATH}/${appId}`);
...
window.shareApp = async (appId, appName) => {
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(shareUrl)
            .then(() => alertMessage('Link copied to clipboard!', 'info'))
            .catch(() => alertMessage(`Copy this link: ${shareUrl}`, 'info'));
    } else {
        alertMessage(`Copy this link: ${shareUrl}`, 'info');
    }
};
...
window.onload = () => {
    ...
    if ('serviceWorker' in navigator && !isLocalhost) {
        ...
        navigator.serviceWorker.getRegistrations()
            ...
            .then(() => console.log('Service workers disabled for local development'))
            .catch(err => console.error('Failed to disable local service workers', err));
    }
};
